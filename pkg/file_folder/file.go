...
// 在 Golang 中，写 文件 有四种方法，分别为：
//使用 io.WriteString 写文件，在写入文件之前，我们需要判断文件是否存在，如果文件不存在，则需要创建文件。
//使用 ioutil.WriteFile 写文件，在写入文件之前，我们不需要判断文件是否存在，如果文件不存在，会自动创建文件，如果文件存在，则会覆盖原来的内容。
//使用 file.Write 写文件，在写入文件之前，我们需要判断文件是否存在，如果文件不存在，则需要创建文件。
//使用 writer.WriteString 写文件。在写入文件之前，我们需要判断文件是否存在，如果文件不存在，则需要创建文件。
