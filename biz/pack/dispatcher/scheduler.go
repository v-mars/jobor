...
// - roundRobin
// get rpc conn
func TryGetGrpcClientConn(ctx context.Context, worker JoborWorker) (*grpc.ClientConn, *model.JoborWorker, error) {
	var (
		err  error
		conn *grpc.ClientConn
	)
	for i := 0; i < rpc_biz.DefaultMaxRetryGetWorker; i++ {
		w := worker()
		if w == nil {
		...
		if err != nil {
			hlog.Errorf("GetGrpcClientConn addr[%s] failed: %s", w.Addr, err)
			continue
		}
...
func getGrpcClientConn(ctx context.Context, addr string) (*grpc.ClientConn, error) {
	...
	if false {
		...
		if err != nil {
			hlog.Errorf("credentials.NewClientTLSFromFile failed: %s", err)
			return nil, err
		}
