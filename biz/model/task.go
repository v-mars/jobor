...
type JoborTask struct {
	...
	Notify        task.Notify   `gorm:"type:mediumtext;null;comment:告警通知，格式：json" json:"notify" form:"notify"`
...
func (u *JoborTask) GetTaskRpc() *task.TaskResp {
	rpcObj := task.TaskResp{}
...
func (u *Tasks) ListTask() (uis []*task.TaskResp) {
	if u != nil {
...
func WhereTask(req *task.TaskQuery) func(Db *gorm.DB) *gorm.DB {
	return func(Db *gorm.DB) *gorm.DB {
		var sql = "name like ?"
...
func AddTask(ctx context.Context, Db *gorm.DB, req *task.PostTaskReq) (JoborTask, error) {
	var row JoborTask
...
func ModTask(ctx context.Context, Db *gorm.DB, _id interface{}, req *task.PutTaskReq) (JoborTask, error) {
...
func GetTaskInfoById(id interface{}, isPanic bool) (*task.TaskResp, error) {
	var err error
	var u task.TaskResp
...
func GetTaskInfoByName(name string, isPanic bool) (task.TaskResp, error) {
	var err error
	var u task.TaskResp
