...
func NewCasbin(c *conf.Config) (*casbin.Enforcer, error) {
	//dbSourceName := fmt.Sprintf("%s:%s@tcp(%s:%s)/%s", "ocean", "123456","127.0.0.1","3306", "ops")
	adapter, err := gormAdapter.NewAdapter("mysql", c.MySQL.DSN, true) // Your driver and data source.
	if err != nil {
		return nil, err
	}
	...
	// localhost:6379
	w, err := rediswatcher.NewWatcher(fmt.Sprintf("%s", c.Redis.Address),
	...
	if err = w.SetUpdateCallback(callbackLoadPolicy); err != nil {
...
func callbackLoadPolicy(val string) {
	hlog.Infof("callback [%s] casbin loadpolicy start", val)
	if err := Enforcer.LoadPolicy(); err != nil {
		hlog.Errorf("callback casbin loadpolicy err: %s", err)
	}
	hlog.Infof("callback casbin loadpolicy success")
}
