...
import (
	...
	"jobor/internal/app/controllers/jobor/define"
...
func CronAddFuncCommon(evt,trigger string, t tbs.JoborTask)  {
	...
	defer func() {
		...
		if err= db.DB.Model(&taskLog).Omit([]string{"Ctime", "Mtime"}...).Updates(MapData).Error;err!=nil{
	...
	var executor = DataCode{Lang: Lang(t.Lang),ScriptCode: t.Data}
