...
调度任务系统一般负责触发任务执行的触发，而执行任务的一般是业务系统，这个时候，很多任务对于调度系统来说都是阻塞的。
所以调度系统一般是IO密集型而非CPU密集型。java开发调度系统，需要非常多的线程，一般会达到几百，甚知几千几万个线程。
CPU大量的工作用来做系统线程切换。go的goroutine非常适和任务多，IO密集的调度系统。
